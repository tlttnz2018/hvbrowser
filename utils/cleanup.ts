...
const replaceAll: Array<string | RegExp> = [
  // 长文字替换
...
const oneWordReplace: Record<string, string> = {
  'b[āà]ng': '棒',
  bào: '爆',
  bà: '吧',
  bī: '逼',
...
const replaceFix: Record<string, string> = {
  // ===误替换还原===
  碧欲: '碧玉',
  美欲: '美玉',
  欲石: '玉石',
...
// Put oneWordReplace, replaceFix to replace.
function extendRule(replaceRule: Record<string, string>) {
  Object.entries(oneWordReplace).forEach(([key, value]) => {
    replaceRule['\\b' + key + '(?![a-z])'] = value;
  });
  Object.assign(replaceRule, replaceFix);
}

extendRule(replace);

const removeLineRegExp = /<p>[　\s。;，！\.∷〖]*<\/p>/g;
const replaceBrs = /(<br[^>]*>[ \n\r\t]*){1,}/gi;

function handleContentText(text: string): string | null {
  if (!text) return null;
...
function contentReplacements(text: string, rule: Record<string, string>): string {
  if (!text) return text;
...
function replaceText(text: string, rule: unknown): string {
  if (rule instanceof RegExp) {
    return text.replace(rule, '');
  } else if (typeof rule === 'string') {
    let ruleStr = rule as string;
    Object.entries(CHAR_ALIAS).forEach(([key, value]) => {
      ruleStr = ruleStr.replace(key, value);
    });
    return text.replace(new RegExp(ruleStr, 'ig'), '');
    ...
    const objRule = rule as Record<string, string>;
    for (const key in objRule) {
      text = text.replace(new RegExp(key, 'ig'), objRule[key]);
    }
    return text;
  }
  return text;
...
function toRE(obj: string | RegExp, flag?: string): RegExp {
...
export async function cleanupHtml(text: string): Promise<string | null> {
  return handleContentText(text);
}
...
export async function updateRelativeUrl(text: string, baseUrl: string): Promise<string> {
  ...
  const tags = findAll((elem) => (elem as Element).name in urlAttrs, doc.children) as Element[];
