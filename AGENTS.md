...
- Offline next-chapter prework is allowed only as bounded background conversion/shaping for the next known downloaded chapter. Keep full HTML out of Zustand, retain at most one preload within explicit byte budgets, and reuse `utils/offline-chapter-preload.ts` plus the guarded Worklet scheduler.
...
- Reader Worklets are enabled through Expo SDK 54's `babel-preset-expo`, Metro `inlineRequires`, and a guarded native-only `hvbrowser-reader` runtime. Reader HTML shaping and pure Han-Viet conversion are worker-routed with timeout/RN fallbacks. Do not run current-reader search through Worklets on SDK54; both index prewarm and cold manual search crashed native. Encoding, cleanup, SQLite definition lookup, search matching, and WebView commands stay on RN/native paths. Use `.agents/skills/worklet-migration/SKILL.md` before moving more work to Worklets.
...
- [`utils/offline-chapter-preload.ts`](/Users/saigon/dev/hvbrowser/utils/offline-chapter-preload.ts): bounded next-offline-chapter conversion/base-href preloading
